...
class CNSST:
    ...
    def fast_plot(self):
        ...
        self.array_2 = position_index(self.X.astype(int)).copy()  # added
        _x = rearrange(self.F, self.array[:,0]) # row
        _y = rearrange(self.F, self.array[:,1]) # column
        R = rearrange(self.F, self.array_2[:,2]) # added 
        G = rearrange(self.F, self.array_2[:,3]) # added
        B = rearrange(self.F, self.array_2[:,4]) # added
        for m in range(0,len(self.clusters)):
...
#         _x = rearrange(self.F, self.array[:,0]) # row
#         _y = rearrange(self.F, self.array[:,1]) # column
#         R = rearrange(self.F,  self.array[:,2]) # added 
#         G = rearrange(self.F,  self.array[:,3]) # added
#         B = rearrange(self.F,  self.array[:,4]) # added
#         for m in range(0,len(self.clusters)):
